...
class GamePlaySerializer(serializers.ModelSerializer):
    winning_combinations = serializers.ListField(child=serializers.ListField(
        child=serializers.IntegerField(default=None, allow_null=True)
    ))
    winner_combination = serializers.ListField(child=serializers.ListField(
        child=serializers.IntegerField(default=None, allow_null=True)
    ))
    class Meta:
        model = Game
    ...
    def validate_players(self, players):
        ...
        if len(players) > self.initial_data['max_players_number']:
            raise ValidationError(
                "Maximum count of players reached."
            )
        for player in players:
            if player not in login_players:
                raise ValidationError(
                    "Player must be login."
                )
        return players
    ...
    def _setup(self, validated_data):
        ...
        all_possible_moves = [
            Move.objects.create(row=row, col=col) for col in range(validated_data['board_size'])
    ...
    def _get_winning_combinations(self, all_possible_moves):
        rows = [
            list(Move.objects.filter(row=move.row).values_list('id', flat=True)) for move in all_possible_moves[:self.data['board_size']]
        ]
    ...
    def update(self, instance, validated_data):
        if self._is_move_valid(validated_data['move']):
            ...
            if self.is_tied() or self.has_winner:
                ...
                moves = self.current_moves.objects.filter(player=self.current_player).count()
