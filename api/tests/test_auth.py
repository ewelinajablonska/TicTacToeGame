...
def test_view_detail_failure_without_authentication(client):
    ...
    response = client.get(reverse("gameplay-detail", kwargs={"pk": 1}))
    assert response.status_code == HTTP_401_UNAUTHORIZED
...
# @pytest.mark.django_db
# def test_view_detail_failure_without_authentication(client):
#     response = client.get(reverse('gameplay-list'), kwargs={'pk':1})
#     assert response.status_code == HTTP_401_UNAUTHORIZED

# response = client.get(reverse('rest_login'), data=dict(username='first_user',email='first_user@gmail.com',password='first_password'))
